...
public class Chocken extends Animal {
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && !this.isChickenJockey() && --this.eggTime <= 0) {
            ...
            this.spawnAtLocation(CreepositoryItems.CHOCOLATE_EGG);
    ...
    public Chocken getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return CreepositoryEntities.CHOCKEN.get().create(level);
    }
    ...
    static {
        BABY_DIMENSIONS = CreepositoryEntities.CHOCKEN.get().getDimensions().scale(0.5F).withEyeHeight(0.2975F);
    }
}